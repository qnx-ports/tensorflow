...
FallbackState::CreateWithDeviceMgr(
    const SessionOptions &session_options,
    const tensorflow::FunctionDefLibrary &fdef_lib,
    absl::Nonnull<StaticDeviceMgr *> device_mgr) {
...
FallbackState::FallbackState(const SessionOptions &session_options,
    ...
    : session_options_(session_options),
      ...
      device_manager_ptr_(
          std::holds_alternative<absl::Nonnull<StaticDeviceMgr *>>(device_mgr)
              ? std::get<absl::Nonnull<StaticDeviceMgr *>>(device_mgr)
              : &device_manager_),
