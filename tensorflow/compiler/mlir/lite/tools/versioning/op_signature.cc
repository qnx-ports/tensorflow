...
using tflite::QuantizationParameters;
  ...
  for (int32_t i = 0; i < tensors->size(); ++i) {
    ...
    if (tensor_no >= 0) {
      if (subgraph->tensors() && tensor_no < subgraph->tensors()->size()) {
        ...
        auto _ = ConvertTensorType(fb_tensor->type(), &tensor_spec.type);
        ...
        if (shape_vec) {
          for (int32_t j = 0; j < shape_vec->size(); ++j) {
        ...
        if (shape_signature_vec) {
          for (int32_t j = 0; j < shape_signature_vec->size(); ++j) {
...
OpSignature GetOpSignature(const OperatorCode* op_code, const Operator* op,
  ...
  switch (builtin_code) {
    ...
    case tflite::BuiltinOperator_CONV_2D: {
      ...
      if (filter_quant && filter_quant->scale() &&
          filter_quant->scale()->size() &&
          filter_quant->scale()->size() == num_filters) {
