...
TEST_F(CutlassFusionTest,
       GemmWithUpcastWithLhsContractingDimensionNotOneShouldNotBeFused) {
  const char* hlo = R"(
  ENTRY e {
    // Column major operand.
    ...
    c1 = f32[32,8]{1,0} convert(p1)
    ROOT dot = f32[16,8]{1,0} dot(p0, c1),
      lhs_contracting_dims={0}, // Zero lhs contracting dimension.
      rhs_contracting_dims={0}
  })";
  CustomKernelFusionPatternRegistry patterns;
  patterns.Emplace<CutlassGemmWithUpcastPattern>();
...
TEST_F(CutlassFusionTest,
       GemmWithUpcastWithRhsContractingDimensionNotOneShouldNotBeFused) {
  const char* hlo = R"(
  ENTRY e {
    // Column major operand.
    p0 = f32[16,32]{1,0} parameter(0)
    ...
    ROOT dot = f32[16,8]{1,0} dot(p0, c1),
      lhs_contracting_dims={1}, 
      rhs_contracting_dims={1} // Zero rhs contracting dimension.
  })";
  CustomKernelFusionPatternRegistry patterns;
  patterns.Emplace<CutlassGemmWithUpcastPattern>();
  auto device = TestGpuDeviceInfo::RTXA6000DeviceInfo();
